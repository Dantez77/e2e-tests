...
test.describe('Salidas', () => {
  ...
  test.beforeEach(async () => {
    ...
    await inventarioPage.goToSubModule(InventarioPage.MAIN.SALIDAS);
    iframe = page.frameLocator('iframe');
  });
  ...
  test('Grabar salida de inventario', async () => {
    ...
    await iframe.locator('[role="option"][data-index="2"]').click(); // INDEX=0: Almacen 1
  ...
  test('Modificar salida de inventario', async () => {
    ...
    await iframe.getByRole('row', { name: TDate }).first().click();
    await iframe.getByRole('textbox', { name: 'Concepto' }).fill(concepto);
  ...
  test('Anular salida de inventario', async () => {
    const concepto = 'Salida modificada: ' + randomID;
